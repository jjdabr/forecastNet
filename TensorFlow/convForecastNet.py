...
def forecastnet_conv_graph(X, Y, hidden_dim, out_seq_length, is_training):
    ...
    # First cell
    i = 0
    hidden1 = tf.layers.conv1d(inputs=tf.expand_dims(X, axis=2),
                               filters=hidden_dim,
                               kernel_size=2,
                               padding='same', activation=tf.nn.relu,
                               kernel_initializer=tf.contrib.layers.variance_scaling_initializer(),
                               bias_initializer=tf.zeros_initializer(), name='conv' + str(i) + 'a')
    hidden2 = tf.layers.average_pooling1d(inputs=hidden1, pool_size=2, strides=1, name='pool' + str(i) + 'a')
    hidden3 = tf.layers.conv1d(inputs=hidden2,
                               filters=hidden_dim,
                               kernel_size=2,
                               padding='same', activation=tf.nn.relu,
                               kernel_initializer=tf.contrib.layers.variance_scaling_initializer(),
                               bias_initializer=tf.zeros_initializer(), name='conv' + str(i) + 'b')
    hidden4 = tf.layers.average_pooling1d(inputs=hidden3, pool_size=2, strides=1, name='pool' + str(i) + 'b')
    hidden5 = tf.layers.flatten(hidden4, name='flat' + str(i))
    # Add an extra hidden layer before the output
    hidden6 = tf.layers.dense(inputs=hidden5, units=hidden_dim, activation=tf.nn.relu,
                              kernel_initializer=tf.contrib.layers.variance_scaling_initializer(),
                              bias_initializer=tf.zeros_initializer(), name='dense0')
    # hidden_drop = tf.layers.dropout(inputs=hidden, rate=1.-keep_prob, training=is_training, name='dropoutc0')
    # First interleaved output
    output_mu = tf.layers.dense(inputs=hidden6, units=1, activation=None, name='output_mu0')
    output_sigma = tf.layers.dense(inputs=hidden6, units=1, activation=tf.nn.softplus, name='output_sigma0')
    ...
    for i in range(1, out_seq_length):
        ...
        concat = tf.cond(is_training,
                         ...
                         lambda: tf.concat((hidden6, X, tf.slice(Y, [0, i - 1], [-1, 1])), axis=1,
                                           name='concat' + str(i)),
        ...
        # Next cell
        hidden1 = tf.layers.conv1d(inputs=tf.expand_dims(concat, axis=2),
                                   filters=hidden_dim,
                                   kernel_size=2,
                                   padding='same', activation=tf.nn.relu,
                                   kernel_initializer=tf.contrib.layers.variance_scaling_initializer(),
        ...
        hidden6 = tf.layers.dense(inputs=hidden5, units=hidden_dim, activation=tf.nn.relu,
                                  kernel_initializer=tf.contrib.layers.variance_scaling_initializer(),
                                  bias_initializer=tf.zeros_initializer(), name='dense' + str(i))
        # Next interleaved output
        output_mu = tf.layers.dense(inputs=hidden6, units=1, activation=None, name='output_mu' + str(i))
        output_sigma = tf.layers.dense(inputs=hidden6, units=1, activation=tf.nn.softplus, name='output_sigma' + str(i))
        output = tf.cond(is_training,
                         lambda: output_mu,
                         lambda: gaussian_sample(output_mu, output_sigma),
                         name='output' + str(i))
...
def forecastnet_conv_graph2(X, Y, hidden_dim, out_seq_length, is_training):
    ...
    for i in range(1, out_seq_length):
        ...
        hidden6 = tf.layers.dense(inputs=hidden5, units=hidden_dim, activation=tf.nn.relu,
                                  ...
                                  bias_initializer=tf.zeros_initializer(), name='dense' + str(i))
        # Next interleaved output
        output = tf.layers.dense(inputs=hidden6, units=1, activation=None, name='output'+str(i))
        outputs.append(output)
    # Convert the outputs list to a tensor
    outputs = tf.concat(outputs, axis=1, name='outputs')
