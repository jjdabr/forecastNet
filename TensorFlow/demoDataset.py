"""
Functions to generate the synthetic dataset used for the time-invariance test
in section 6.1 of the ForecastNet paper.
...
def time_series(t, f=0.02):
    """
    Generate time series data over the time vector t.
    The value of t can be a sequence of integers generated
    using the numpy.arange() function.
    The default frequency is designed for 2750 samples.
...
def generate_data(T=2750, period=50, n_seqs=4):
    """
    Generate a dataset using the time_series function.
    The function generates a dataset comprising 'n_seqs'
    time-series sequences of length T.
    This dataset is split into training, testing, and validation sets.
    Returns a training, a test, and validation dataset.
